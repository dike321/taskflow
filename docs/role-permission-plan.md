...
- **`companyId` sendiri tetap cuma field pencatatan** untuk internal (semua user internal berbagi company id 1, jadi tidak ada yang perlu di-scope di antara mereka). `warehouseId` **[Selesai, sudah full view-scoping]** — lihat `docs/inventory-module-plan.md` roadmap #1: bukan cuma form create lagi, tapi juga List/History/Batches/Items dikunci ke warehouse staff (disinkronkan 2026-09-21). Supplier Portal (di bawah) juga butuh scoping berbasis `companyId`, tapi itu halaman baru yang terisolasi, bukan perubahan ke `CompaniesPage.tsx`/model permission umum.
...
- **Aksi tulis "Confirm Shipment" — [Selesai, disinkronkan 2026-09-21]**: `StockTransaction.supplierConfirmedAt?: string` (baru) — supplier klik "Confirm Shipment" di tabel My Deliveries, independen dari status approval internal (bisa dikonfirmasi kapan saja, tidak mengubah `status`/stok). Begitu dikonfirmasi, `StockHistoryPage.tsx` di sisi internal menampilkan "Supplier confirmed {tanggal}" kecil di kolom Reference, supaya staff tahu supplier sudah kirim tanpa perlu buka Supplier Portal. Tercatat di Activity Log module `supplierPortal`. Masih di luar scope: aksi tulis lain (update ETA, upload dokumen pengiriman, dll) — cuma satu langkah konfirmasi biner, bukan tracking pengiriman penuh.
